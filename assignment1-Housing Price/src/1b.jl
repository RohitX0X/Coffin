...
x2_tot=x1_tot.^2
x4_tot=x3_tot.^2
x5_tot=x1_tot.*x3_tot
X_tot = cat(x0_tot, x1_tot, x2_tot,x3_tot,x4_tot,x5_tot, dims=2)
X_tot=normalized(X_tot,m_tot)
X_tot[:,1]=ones(m_tot)
X=X_tot[1:17291,:]
# print(X.shape)
...
learningRate = 1
newB, costHistory = gradientDescent(X, Y, B, learningRate, 102000)
YPred = X * newB
rmss=RMSS(X_test,Ytest,newB)
r2=R2(X_test,Ytest,newB,Ymean)
# print("the RMSS score for poly regression is",":",rmss,"  ","the R2 score is",":",r2)
# plot(YPred[1:100])
# plot!(Y[1:100])
# plot(costHistory)
using DataFrames
